...
def get_nix_store(list_file: str) -> Dict[str, Any]:
    ...
    file. The plain should be a <package>-<version> line separated list,
...
def get_discrepancies(upstream, local) -> List[Tuple[str, str, str, str]]:
    ...
    for pname, pconfig in local.items():
        ...
        # Adding the upstream results to the comparison table
        updates.append(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="nixpkgs=https://github.com/NixOS/nixpkgs/archive/nixpkgs-unstable.tar.gz",
    )
    parser.add_argument(
        "--env",
        "-e",
    ...
    has_system_update = sum([x[1] != x[3] for x in system_updates]) == 0
    user_updates = get_discrepancies(upstream, user)
    has_user_update = sum([x[1] != x[3] for x in user_updates]) == 0
    ...
    if has_user_update:
        ...
        print("Updates available for user-space packages!")
        display_updates(user_updates)

    if has_system_update or has_user_update:
